...
func New(db *gorm.DB, rdb *redis.Client, cfg *config.Config) (*gin.Engine, []system.ApiEntry, *crm.IntentQueue, *observability.Metrics) {
	...
	modelCredential := crm.NewModelCredentialHandler(db, cfg.LLM.CredentialEncryptionKey)
	a.privilegedPerm("GET", "/crm/intent/model-credential", "crm:intent:config", modelCredential.List)
	a.privilegedPerm("POST", "/crm/intent/model-credential", "crm:intent:config", modelCredential.Create)
	a.privilegedPerm("PUT", "/crm/intent/model-credential/:id/rotate", "crm:intent:config", modelCredential.Rotate)
	...
	a.privilegedPerm("PUT", "/crm/intent/model-config/:id/credential", "crm:intent:config", modelConfig.BindCredential)
	...
	promptConfig := crm.NewPromptHandler(db, cfg.LLM)
	a.privilegedPerm("GET", "/crm/intent/prompt", "crm:intent:config", promptConfig.List)
	a.privilegedPerm("POST", "/crm/intent/prompt", "crm:intent:config", promptConfig.Create)
	a.privilegedPerm("PUT", "/crm/intent/prompt/:id", "crm:intent:config", promptConfig.Update)
	a.privilegedPerm("POST", "/crm/intent/prompt/:id/quality-gate", "crm:intent:config", promptConfig.QualityGate)
	a.privilegedPerm("POST", "/crm/intent/prompt/:id/canary", "crm:intent:config", promptConfig.Canary)
	a.privilegedPerm("POST", "/crm/intent/prompt/:id/activate", "crm:intent:config", promptConfig.Activate)
	a.privilegedPerm("POST", "/crm/intent/prompt/rollback", "crm:intent:config", promptConfig.Rollback)
	a.privilegedPerm("GET", "/crm/intent/prompt/:id/changes", "crm:intent:config", promptConfig.Changes)
